...
// Fetch product data that matches the search term
$sql = "SELECT * FROM product WHERE name LIKE '%$searchTerm%' OR brand LIKE '%$searchTerm%' OR classification LIKE '%$searchTerm%'";
...
<main style="background-color: ghostwhite; padding-bottom: 50px; margin-right: 20px; margin-left: 20px; margin-top: 30px;">
  ...
  <h2 align="center" style=" margin-bottom: 0px; font-family: arial;font-weight: bold;">PRODUCTS</h2>
  </div>
      ...
      while ($row = $result->fetch_assoc()) {
          ...
          echo '<div class="product-card">
                <div class="product-tumb">
                  <img src="img/products/' . $row["main_image"] . '" alt="' . $row["name"] . '">
                </div>
                <div class="product-details">
                  <span class="product-catagory">' . $row["classification"] . '&nbsp  —  &nbsp' . $row["brand"] . '</span>
                  <h4><a href="product-details.php?id=' . $row["serial_id"] . '">' . $row["name"] . '</a></h4>
